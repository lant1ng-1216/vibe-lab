...
/* The built-in "prism" palette is only cyanâindigoâmagenta, so a sweep
 ...
 * Type @ or / to open the menus; ââ + Enter to pick.
 * Variants: Rounded (card radius) Â· Pill (full radius).
 * âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ */
...
const AUTO_STEPS: {
  ...
  { draft: "", hold: 800 },
  // open the model picker and upgrade to the flagship â rainbow sweep
...
export default function PromptBar({
   ...
   * toggling its own background â matches the gliding pill in the nav */
  useLayoutEffect(() => {
    const target = rowRefs.current[active];
    if (target) setRowBox({ top: target.offsetTop, height: target.offsetHeight });
  }, [menu, query, active, connected, rows.length]);

  /* same gliding highlight in the model menu â floats to the hovered
   ...
   * colour on every reload â pin it so the rainbow is identical each time. */
  const makeShader = () => {
  ...
  const celebrate = () => {
    ...
    // Recreate the shader per sweep so uTime restarts at 0 â the hue phase
  ...
  return (
    ...
    >
      {/* composer is the anchor â menus grow up from its top edge */}
      <div ref={composerAnchorRef} className="relative">
      {/* ââ @ / slash menu âââââââââââââââââââââââââââââââ */}
      {menu && (
        <div
        ...
        >
          {/* single gliding highlight â appears once a row is hovered */}
          <span
            aria-hidden
      ...
      {/* ââ model menu âââââââââââââââââââââââââââââââââââ */}
      {modelOpen && (
        <div
        ...
        >
          {/* single gliding highlight â floats to the hovered / selected row */}
          <span
            aria-hidden
            className="pointer-events-none absolute inset-x-1 rounded-[6px] bg-hover"
            style={{
      ...
      {/* ââ composer âââââââââââââââââââââââââââââââââââââ */}
      <div
      ...
      >
        {/* rainbow glimm sweep â plays across the interior on model change.
        ...
        >
          ...
          <textarea
            ...
            placeholder={listening ? "Listeningâ¦" : placeholder ?? "Write a messageâ¦"}
            aria-label="Prompt"
          ...
          {/* send â tactile square (round in the pill variant) */}
          <button
            type="button"
            aria-label="Send"
            disabled={!canSend}
