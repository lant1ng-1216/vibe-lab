...
const ROWS: ToolStep[] = [
  {
    icon: "think", label: "Thinking", chip: "Planning the churn scheduleâ¦", mono: false, detailMono: false,
    detail: [
  ...
  },
  {
    ...
    detail: [
      { text: "â built in 1.2s" },
      { text: "â 34 checks passed" },
    ],
  },
  {
    icon: "read", label: "Read image", chip: "flavor-chart.png", mono: true, detailMono: false,
    detail: [
      { text: "1280 Ã 720 Â· line chart, three summers." },
...
/* hovering a file chip opens its diff â green added, red removed */
          ...
          {diffs.map((d, i) => (
            ...
            >
              ...
              >
                ...
                {d.del > 0 && <span className="shrink-0 text-red tabular-nums">â{d.del}</span>}
              </button>
        ...
        >
          <div className="flex items-center justify-between border-b border-line px-2.5 py-1.5 font-mono text-[11px]">
            ...
            <span className="shrink-0 tabular-nums">
              ...
              {(diffs.find((diff) => diff.file === preview.file)?.del ?? 0) > 0 && (
                <span className="text-red"> â{diffs.find((diff) => diff.file === preview.file)?.del}</span>
              )}
            </span>
          </div>
          <div className="py-1 font-mono text-[11px] leading-[1.8]">
            {(diffLines[preview.file] ?? []).map((line, index) => (
              ...
              >
                <span className="w-3 shrink-0 select-none">{line.tone === "add" ? "+" : line.tone === "del" ? "â" : " "}</span>
